...
export function DemoRoleSwitcher({ currentRole }: { currentRole: DemoRole }) {
  return (
    <form action={setDemoRole} className="ui-demo-role-switcher grid gap-2 border-t border-border bg-surface-soft px-5 py-4">
      <label htmlFor="demo-role" className="text-xs font-semibold uppercase text-muted-foreground">
        Demo role
      </label>
      <select
        id="demo-role"
